...
@inline molmass_dryair(p::SWP) = molmass_dryair(p.parameters)
@inline molmass_water(p::SWP)  = molmass_water(p.parameters)
@inline molmass_ratio(p::SWP)  = molmass_ratio(p.parameters)
...
@inline LH_f0(p::SWP)          = LH_f0(p.parameters)
@inline T_freeze(p::SWP)       = T_freeze(p.parameters)
@inline T_triple(p::SWP)       = T_triple(p.parameters)
@inline T_icenuc(p::SWP)       = T_icenuc(p.parameters)
@inline pow_icenuc(p::SWP)     = pow_icenuc(p.parameters)
@inline press_triple(p::SWP)   = press_triple(p.parameters)
@inline T_0(p::SWP)            = T_0(p.parameters)

@inline e_int_v0(p::SWP)       = LH_v0(p) - R_v(p) * T_0(p)

@inline cp_v(p::SWP)           = cp_v(p.parameters)
@inline cp_l(p::SWP)           = cp_l(p.parameters)
@inline cp_i(p::SWP)           = cp_i(p.parameters)

@inline cv_l(p::SWP)           = cv_l(p.parameters)
@inline cv_i(p::SWP)           = cv_i(p.parameters)

@inline kappa_d(p::SWP)        = kappa_d(p.parameters)
